    ...
    suspend fun load(): StoreSnapshot? {
        ...
        return StoreSnapshot(
            ...
            entries = dao.entries().map {
                DayEntry(it.id, it.t, it.d, it.amt, it.cls, it.customerId, it.debtDelta, it.day, it.saleAmount, it.cashAmount, it.stockDelta, it.trialAmount, it.lines)
            },
    ...
    suspend fun save(s: StoreSnapshot) {
        dao.replaceAll(
            ...
            entries = s.entries.mapIndexed { i, x ->
                EntryRow(x.id, x.t, x.d, x.amt, x.cls, x.customerId, x.debtDelta, x.day, x.saleAmount, x.cashAmount, x.stockDelta, x.trialAmount, x.lines, i)
            },
